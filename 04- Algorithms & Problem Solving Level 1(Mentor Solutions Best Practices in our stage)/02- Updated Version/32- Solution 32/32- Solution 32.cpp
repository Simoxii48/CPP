...
4. Power Calculation Function (PowerOfM)
    ...
    If the exponent is 0, it returns 1 (since any number raised to 0 is always 1).
    Uses a for loop to compute the power by repeated multiplication:
    P= N^M
    Returns the computed power value.

5. Program Execution (main())
    Calls ReadNumber() to get user input for the base.
    Calls ReadPower() to get user input for the exponent.
    Calls PowerOfM() to compute the power.
    Prints the result.
